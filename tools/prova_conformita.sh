...
for n in 2 5 15; do
    prova "$n fotogrammi, niente B" "$SRC1" 176x144 \
        -frames:v $n -c:v libx264 -profile:v main -qp 26 -bf 0 -g 30 -x264opts weightp=0
done
prova "10 fotogrammi, 3 riferimenti" "$SRC1" 176x144 \
    -frames:v 10 -c:v libx264 -profile:v main -qp 26 -bf 0 -refs 3 -g 30 -x264opts weightp=0
prova "10 fotogrammi, high 8x8" "$SRC1" 176x144 \
    -frames:v 10 -c:v libx264 -profile:v high -qp 26 -bf 0 -g 30 -x264opts weightp=0
...
prova "10 fotogrammi con B" "$SRC1" 176x144 \
    -frames:v 10 -c:v libx264 -profile:v main -qp 26 -bf 2 -g 30 -x264opts weightp=0:weightb=0

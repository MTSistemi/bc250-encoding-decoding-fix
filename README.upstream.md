...
> [!TIP]
> **Dynamic GPU Load Balancing & Zero-Stutter Frame Pacing**: Compute shader encoders naturally contend with 3D games running on the same CUs. To prevent stream frame drops when games saturate the GPU, this driver includes a **Real-Time Dynamic Saturation Governor**. Under mild GPU contention ($\ge 8\text{ ms}$), the governor seamlessly transitions to Tier 1 (GPU Fast ME), reducing search radius on the GPU's 40 CUs in $\sim 1\text{ ms}$ with zero CPU bus stalls. Fast 4-frame hysteresis recovers back to Tier 0 as soon as contention clears. Tier 2 CPU SIMD ME is available as an opt-in toggle (`BC250_ENABLE_CPU_ME=1`).
>
> - **Live Telemetry**: Set `BC250_GOVERNOR_STATS=60` to log real-time governor tier, GPU latency, and offload counts every 60 frames.
> - **Fast Hysteresis**: Default 4-frame hysteresis ensures instantaneous recovery from scene changes and transient latency spikes.
> - **Setup Guide**: See the complete [Sunshine & Moonlight Setup Guide](docs/sunshine-guide.md).
>
> **Every throughput figure here is measured with an otherwise idle GPU.** Under heavy 3D titles, the dynamic governor automatically adapts to preserve stream frame pacing.
...
- **H.265/HEVC Architecture & Streaming Advice**: HEVC Main profile encoding is fully functional via VA-API (`VAProfileHEVCMain` / `VAEntrypointEncSlice`) with periodic/forced IDR I-slices, inter-predicted P-slices, strictly spec-compliant 5-candidate spatial merge skip (matching ITU-T H.265 Section 8.5.3.2.2 for bit-exact hardware decoder compatibility with Qualcomm/Apple/Intel/Nvidia decoders), rate control (CQP/VBR/CBR), quality presets (1–7), max frame size limits, dynamic governor parity, and AVX2/SSE2 vectorization. The HEVC PPS explicitly disables the in-loop deblocking filter (`pps_deblocking_filter_disabled_flag=1`) to ensure encoder and decoder reference frames match exactly — without this, P-frame inter prediction drifts because the decoder deblocks its reference but the encoder does not. Because HEVC's 4x4 DST-VII/DCT-II transform and CABAC entropy coding currently execute on the host CPU rather than compute shaders, real-time 1080p60 encoding incurs higher CPU load than H.264. For low-latency real-time game streaming (Sunshine/Moonlight), H.264 remains strongly recommended.
